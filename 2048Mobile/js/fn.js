var board=new Array(),
	...
	cellSize=0.18*documentWidth,
	paddingSize=0.04*documentWidth;
	
fn={
	getPosTop:function(i,j)	{
		return paddingSize + (paddingSize+cellSize) * i
	},
	getPosLeft:function(i,j){
		return paddingSize + (paddingSize+cellSize) * j
	},
	newgame:function(){
			...
			for (var j=0; j<4;j++) {
				...
				if(!board[i][j]){
					theNumberCell.css({
						"width":"0",
						"height":"0",
						"top":fn.getPosTop(i,j)+cellSize/2,
						"left":fn.getPosLeft(i,j)+cellSize/2
					})
				}
				else{
					theNumberCell.css({
	...
	noMove:function(arr){
		if(fn.canMoveLeft(arr) || fn.canMoveRight(arr) || fn.canMoveUp(arr) || fn.canMoveDown(arr))
			return false;
		return true
	}	
}
